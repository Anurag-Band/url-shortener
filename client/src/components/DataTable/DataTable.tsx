...
const DataTable: React.FunctionComponent<IDataTableProps> = (props) => {
  ...
  const renderTableData = () => {
    return data.map((item) => {
      return (
        <tr
          key={item._id}
          className="border-b text-white bg-gray-600 hover:bg-gray-300 hover:text-gray-900"
        >
  ...
  const copyToClipboard = async (url: string) => {
    try {
      ...
      alert(`URL copied: ${serverUrl}/api/shortUrl/${url}`);
    } catch (error) {
      console.log(error);
    }
  };
      ...
      <div className="relative overflow-x-auto shadow-sm sm:rounded-lg">
        <table className="w-full table-fixed text-sm text-left rtl:text-right text-gray-500">
          <thead className="text-md uppercase text-gray-50 bg-gray-700">
            <tr>
              <th scope="col" className="text-lg px-6 py-3 w-6/12">
                Full Url
              </th>
              <th scope="col" className="text-lg px-6 py-3 w-3/12">
                Short Url
              </th>
